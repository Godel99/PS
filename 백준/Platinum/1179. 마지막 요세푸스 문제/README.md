...
# 풀이
간단하게 큐를 이용해 요세푸스 문제를 해결할 수 있지만, 이 문제의 경우 입력값의 범위가 매우 크기 때문에 큐로 접근했다간 시간초과(TLE)가 납니다.
### 수학적 접근
요세푸스 문제를 수학적 접근을 해본다면, 한 바퀴를 돌았을 경우 그 다음은 좀 더 작은 요세푸스 문제가 된다는 점을 알 수 있습니다. 이를 점화식으로 표현하면 아래와 같습니다.

$$f(n,k)=(f(n−1,k)+k)\ mod \ n$$
### $n$ 줄이기
$n<k$ 경우 $n$이 크지 않아 위 점화식을 이용해 간단히 해결하면 됩니다. 그러나 여전히 $\mathcal{O}(n)$입니다.


문제는 $n>k$이 클 경우 이를 어떻게 줄일 수 있느냐입니다. 다행히도 한 바퀴를 돌 경우 $n/k$명이 죽는다는 사실을 착안하여 $n-n/k$로 $n$을 줄여갈 수 있습니다.

줄이는 과정은 **재귀**를 이용해 간단히 구현해 봅시다.
