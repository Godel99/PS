...
* 예를 들어 abc, ab 등 문자열은 중복된 구간이 많은 데이터 구조입니다. a라는 시작점을 저장 후 b라는 중복된 결과만 가지고 있으면 abc, bc를 전부 저장할 필요 없이 abc, bc를 구별할 수 있게 됩니다. 즉, 문자열 자체가 아니라 정수 단위로 메모리를 효율적으로 관리함과 동시에 탐색이나 정렬도 간단하게 할 수 있게 됩니다.
### 기수 정렬과 계수 정렬
...
* 3개의 for문의 의미(r[i+d]자릿)
1. 각 문자의 개수를 새는 것입니다.
2. 그 개수를 누적합니다.
3. 누적합을 이용해 idx에 정렬을 합니다.
* 마찬가지로 r[i] 자릿수에도 동일하게 계수정렬을 하고 두 자릿수(r[i+d], r[i])를 합쳐 지수 정렬을 합니다.
...
* 그렇게 구한 것을 doubling 알고리즘을 핵심인 이전 정렬 값을 이용해 앞에 정렬에 사용하기 위해서 r = nr 하게 됩니다.
```python
global rev, ret
